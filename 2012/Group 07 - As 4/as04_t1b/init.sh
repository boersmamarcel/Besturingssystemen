...
while true; do
	...
	cut -d: -f1 /etc/passwd | grep "$teacherName" > /dev/null
	if [ $? -eq 0 ]; then
...
# Set that ACL thing...
sudo setfacl -m "d:u:$teacherName:rw" $topFolder
...
while true; do
	...
	if [ "$add" == "y" ]  || [ "$add" == "Y" ]; then
		...
		if [ -d "$topFolder/$name" ]; then
			...
			# Add the student as well!
			setfacl -m "d:u:$name:rw" "$topFolder/$name";
			echo "You added $name";
...
echo "./deadline.sh $topFolder $teacherName" | at "$deadline"
